...
<?php foreach ($students as $s): 
    $ban1 = $marks[$s->roll][1]?? null;
    $ban2 = $marks[$s->roll][2]?? null;
    $eng1 = $marks[$s->roll][3]?? null;
    $eng2 = $marks[$s->roll][4]?? null;
    $mat  = $marks[$s->roll][5]?? null;
    $sci  = $marks[$s->roll][6]?? null;
    $bgs  = $marks[$s->roll][7]?? null;
    $rel  = $marks[$s->roll][8]?? null;
    if($s->religion=='Hindu'){
        $rel = $marks[$s->roll][9]?? null;
    }
   
    $ict = $marks[$s->roll][10]?? null;
    ...
    <tr>
        ...
        <td>
            Total = <?= $marks[$s->roll]['grand_total']?? 0; ?>
            GPA = <?= number_format($marks[$s->roll]['gpa']?? 0,2); ?>
            F=<?= $marks[$s->roll]['fail_count']?? 0; ?>
        </td>
        <td><?php if(isset($position[$s->roll])) echo $position[$s->roll]+1;?></td>
